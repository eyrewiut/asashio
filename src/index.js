...
const server = polka()
	...
	.get('/discord', (_, res) => res.redirect(`https://discordapp.com/api/oauth2/authorize?client_id=${process.env.CLIENT_ID}&redirect_uri=${encodeURIComponent(`http://localhost:${process.env.PORT}${process.env.DISCORD_CALLBACK}`)}&response_type=code&scope=${process.env.SCOPES.split(',').join('%20')}`))
	.get('/discord/callback', async (req, res) => {
		...
		data.append('redirect_uri', `http://localhost:${process.env.PORT}${process.env.DISCORD_CALLBACK}`);
